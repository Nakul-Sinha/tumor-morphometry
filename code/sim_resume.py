"""Resume sim_eval after crash: load sim_A.pt, retrain model B, selection + pseudo-test.
Usage: python sim_resume.py --out runs/sim1 [--epochs 40] [--cpi 4] [--tta 4]
"""
import argparse
import json
import sys
...
def main():
    ...
    # load model A from checkpoint
    model_a = S.UNetR18(pretrained=False).to(S.DEVICE)
    ck = torch.load(out / "sim_A.pt", map_location=S.DEVICE, weights_only=False)
    model_a.load_state_dict(ck["model"])
    model_a.eval()
    tau_a = ck.get("mean_tau", -1)
    print(f"loaded sim_A.pt (val {tau_a:.4f})", flush=True)

    # retrain model B
    tr_b, va_b = split(S.SEED + 1)
    model_b, tau_b = S.train_one(images, cells_by, tr_b, va_b, targets, train_stats,
                                 S.SEED + 1, stop_min=1e9, epochs=args.epochs, tag="B")
    torch.save({"model": model_b.state_dict(), "mean_tau": tau_b}, out / "sim_B.pt")
    models = [model_a]
    if tau_b > 0.25:
        models.append(model_b)

    # cthr + estimator selection on A's val
    va_maps = {}
    for iid in va_a:
        img = images[iid]
    ...
    (out / "sim_results.json").write_text(json.dumps(
        {"results": results, "use_alt": use_alt, "best_cthr": best_cthr,
         "tau_a": float(tau_a), "tau_b": float(tau_b),
